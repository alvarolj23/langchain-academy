...
class Analyst(BaseModel):
    ...
    role: str = Field(
        description="Role of the analyst in the context of cryptocurrency analysis.",
    )
...
class GenerateAnalystsState(TypedDict):
    topic: str  # Cryptocurrency of interest
...
class InterviewState(TypedDict):
    ...
    crypto_topic: str  # Cryptocurrency of interest
...
analyst_instructions = """You are tasked with creating a set of AI analyst personas focused on analyzing the cryptocurrency {topic}. Follow these instructions carefully:

1. Review the cryptocurrency:

{topic}

2. Create analysts for the following areas of analysis: Technical Analysis, Fundamental Analysis, Sentiment Analysis, On-Chain Analysis, Regulatory Analysis.
...
Your focus is on analyzing the cryptocurrency: {topic}.

Your goal is to gain insights related to your specialization, focusing on the future prospects of {topic} and whether it is a good investment in the coming days, weeks, or months.
...
def generate_question(state: InterviewState):
    ...
    crypto_topic = state["crypto_topic"]

    # Generate question
    system_message = question_instructions.format(role=analyst.role, topic=crypto_topic)
...
# Search crypto data
def search_crypto_data(state: InterviewState):

    """Retrieve crypto data relevant to the analyst's question"""
    ...
    analyst = state["analyst"]
    crypto_id = state['crypto_topic'].lower().replace(' ', '-')
    ...
    try:
        # Get market data
        market_url = f'https://api.coingecko.com/api/v3/coins/{crypto_id}'
        ...
        if market_response.status_code == 200:
            ...
            context_pieces.append(f"24h Price Change: {price_change_percentage_24h}%")
        else:
            context_pieces.append('Market data not available.')

        # Fetch historical data for technical analysis
        if analyst.role.lower() == 'technical analyst':
            ...
            if history_response.status_code == 200:
                ...
                # You can process prices to calculate technical indicators
                context_pieces.append("Historical Prices (last 30 days) retrieved for technical analysis.")
            else:
                context_pieces.append('Historical data not available.')

        # Fetch sentiment data
        if analyst.role.lower() == 'sentiment analyst':
            sentiment_url = f'https://api.alternative.me/v2/ticker/{crypto_id}/?convert=USD'
            sentiment_response = requests.get(sentiment_url)
            if sentiment_response.status_code == 200:
                sentiment_data = sentiment_response.json()
                # Extract sentiment data if available
                context_pieces.append("Sentiment data retrieved for analysis.")
            else:
                context_pieces.append('Sentiment data not available.')

        # Fetch on-chain data
        if analyst.role.lower() == 'on-chain analyst':
            # Implement on-chain data retrieval (placeholder)
            context_pieces.append("On-chain data retrieved for analysis.")

        # Fetch regulatory news
        if analyst.role.lower() == 'regulatory analyst':
            # Implement regulatory news retrieval using a news API (placeholder)
            context_pieces.append("Regulatory news retrieved for analysis.")
...
answer_instructions = """You are an expert on {topic}.
...
def generate_answer(state: InterviewState):
    ...
    # Answer question
    system_message = answer_instructions.format(topic=crypto_topic, role=analyst.role, context=context[0])
...
def write_section(state: InterviewState):
    ...
    system_message = section_writer_instructions.format(interview=interview, focus=analyst.description)
...
interview_builder.add_node("search_crypto_data", search_crypto_data)
...
interview_builder.add_edge("ask_question", "search_crypto_data")
interview_builder.add_edge("search_crypto_data", "answer_question")
...
def initiate_all_interviews(state: ResearchGraphState):
    ...
    # Otherwise kick off interviews in parallel via Send() API
        ...
        return [Send("conduct_interview", {
            "analyst": analyst,
            "messages": [HumanMessage(content=f"So you said you were analyzing {topic}?")],
            "crypto_topic": topic,
            "max_num_turns": 2,
            "sections": []
...
report_writer_instructions = """You are a technical writer creating a report on this cryptocurrency:
...
5. Focus on the future prospects of {topic} and provide an analysis of whether it may be a good investment in the coming days, weeks, or months.
